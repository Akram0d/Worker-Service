...
def check(request):
    ...
    if len(errors) > 0:
        ...
        if user: #if there is a user with the input email = true
            logged_user = user[0]#save the user info in a varibale called logged_user
        #check if the password form thelogin form equals the  password in the database
        #converts the password from the form from string to byte and the  password in the database from hash to byte and compares the two of them
        if request.POST['password_login']== logged_user.password:#if the passwords match redirect to success page and save the logged in users info to display them in the success page
